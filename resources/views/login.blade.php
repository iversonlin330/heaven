...
<body class="align">
    <div class="grid">
        <p class="title">24H自動贊助後台管理系統<br>管理員登入</p>
        <form action="{{ url('login') }}" method="POST" class="form login">
            <div class="form_field">
                <label for="login_username"><svg class="icon">
                        <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#user"></use>
                    </svg><span class="hidden">Username</span></label>
                <input id="login_username" type="text" name="account" class="form__input" placeholder="帳號"
                    required>
            </div>
            <div class="form_field">
                ...
                <input id="login_password" type="password" name="password" class="form__input" placeholder="密碼"
                    required>
            </div>
			@if($errors->any())
			<p>{{ $errors->first() }}</p>
